...
# Fetch a page in full before searching it. Piping straight into `grep -q`
# makes grep exit on the first match and hand curl a broken pipe, which turns
# these checks flaky.
body() { curl -s "$@"; }
...
has "$(body -b "$ADMIN_JAR" "$BASE/profile")" "Umuariam" && ok "obowo address shows on profile" || bad "obowo address shows on profile"
has "$(body -b "$ADMIN_JAR" "$BASE/profile")" "18 April" && ok "birthday shows day and month only" || bad "birthday shows day and month only"
has "$(body -b "$ADMIN_JAR" "$BASE/profile")" "1995" && bad "birth year is hidden" || ok "birth year is hidden"
...
has "$(body "$BASE/")" "President" && ok "president shows on the home page" || bad "president shows on the home page"
has "$(body "$BASE/")" "Chidi Okoro" && ok "exco name shows on the home page" || bad "exco name shows on the home page"
...
has "$(body "$BASE/")" "Treasurer" && ok "treasurer shows on the home page" || bad "treasurer shows on the home page"
...
has "$(body "$BASE/news")" "October general meeting" && ok "update lists on the news page" || bad "update lists on the news page"
has "$(body "$BASE/")" "October general meeting" && ok "update shows on the home page" || bad "update shows on the home page"
T=$(csrf "$JAR" /profile)
...
has "$(body "$BASE/events")" "End of year thanksgiving" && ok "event lists on the events page" || bad "event lists on the events page"
has "$(body "$BASE/")" "End of year thanksgiving" && ok "event shows on the home page" || bad "event shows on the home page"
has "$(body "$BASE/events/1")" "11:00 am" && ok "event time renders" || bad "event time renders"
T=$(csrf "$ADMIN_JAR" /admin/events/new)
C=$(status -b "$ADMIN_JAR" -c "$ADMIN_JAR" -X POST "$BASE/admin/events/new" \
...
has "$(body -b "$JAR" "$BASE/members")" "Chidi Okoro" && ok "directory lists members" || bad "directory lists members"
has "$(body -b "$JAR" "$BASE/members?q=Accountant")" "Ngozi Eze" && ok "search by occupation works" || bad "search by occupation works"
has "$(body -b "$JAR" "$BASE/members?office=executives")" "Treasurer" && ok "executive filter works" || bad "executive filter works"
has "$(body -b "$JAR" "$BASE/members?q=%25")" "No members matched" && ok "a wildcard search is escaped" || bad "a wildcard search is escaped"
